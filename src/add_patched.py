...
while frames < 60:
	...
	# If we've found at least one, accumulate encodings
	if face_locations:
		fl = face_locations[0].rect if use_cnn else face_locations[0]
		landmark = pose_predictor(frame, fl)
		enc = np.array(face_encoder.compute_face_descriptor(frame, landmark, 1))
		insert_model["data"].append(enc.tolist())
		if len(insert_model["data"]) >= 5:
			break
